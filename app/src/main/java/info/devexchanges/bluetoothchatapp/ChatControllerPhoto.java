...
public class ChatControllerPhoto {
    ...
    public ChatControllerPhoto(Context context, Handler handler) {
    ...
    public void write(ByteArrayOutputStream blob) {
        ReadWriteThread r;
        synchronized (this) {
    ...
    private void connectionFailed() {
        ...
        ChatControllerPhoto.this.start();
    }
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            ...
            while (state != STATE_CONNECTED) {
                ...
                if (socket != null) {
                    synchronized (ChatControllerPhoto.this) {
                        switch (state) {
                            case STATE_LISTEN:
    ...
    private class ReadWriteThread extends Thread {
        ...
        public void run() {
            ...
            File tmp = null;
            try {
                tmp = File.createTempFile("img", "jpg", Environment.getExternalStorageDirectory());
                FileOutputStream f = new FileOutputStream(tmp);

                int num = 0;
                while ((num = inputStream.read(buffer, num, 1024)) > -1) {
                    f.write(buffer, num, 1024);
                }
//                f.close();
//                inputStream.close();
//                while (true) {
...
//                    } catch (IOException e) {
//                        Log.d("Msg", "Input stream was disconnected", e);
//                        break;
//                    }
//                }
...
//
////                    if(bytes!=-1){
////                        while ((bytes==1024)&&(buffer[1024-1] != 0)) {
////                            totalbytes = totalbytes + inputStream.read(buffer);
////                        }
////                    }
////                    totalbytes = totalbytes + inputStream.read(buffer);
//                    // Send the obtained bytes to the UI Activity
